...
function AIRecommendingStep({ recommendations, llmPowered, llmProvider, onContinue }: { recommendations: Recipe[] | null; llmPowered?: boolean; llmProvider?: string; onContinue: () => void }) {
  if (!recommendations) {
    return (
...
export default function Pipeline() {
  ...
  useEffect(() => {
    ...
    const langs = profile.languages.map((l: any) => l.name.toLowerCase());
    const frameworks = profile.frameworks.map((f: any) => f.name.toLowerCase());
    const deps = (depResult?.dependencies || []) as any[];
      ...
      .then(statusRes => {
        if (statusRes.data?.llm_available) {
          return getLlmRecommendations({
            ...
            if (recs.length > 0) {
              // Map LLM response to Recipe shape expected by the UI
              const mapped = recs.map((r: any) => ({
                id: r.recipe_id,
                name: r.name,
                description: r.reason || '',
                language: 'all',
                category: r.category || 'upgrade',
                complexity: r.risk?.toLowerCase() || 'medium',
                tags: [],
                requires: [],
                conflicts_with: [],
                recommended: r.priority === 'HIGH',
                score: r.priority === 'HIGH' ? 20 : r.priority === 'MEDIUM' ? 12 : 5,
                reasons: [r.reason || ''],
                applicable: true,
              }));
              setLlmPowered(true);
  ...
  const renderContent = () => {
    ...
    switch (stage) {
      ...
      case 'ai-recommending':
        return <AIRecommendingStep recommendations={recommendations} llmPowered={llmPowered} llmProvider={llmProvider} onContinue={() => go('recipe-selection')} />;
